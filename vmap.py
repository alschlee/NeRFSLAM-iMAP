...
def resize_tensor_if_needed(tensor, target_shape, device):
    """Helper function to resize tensor if shapes don't match"""
    if tensor.shape != target_shape:
        import cv2
        resized = cv2.resize(tensor.cpu().numpy(), (target_shape[1], target_shape[0]), interpolation=cv2.INTER_NEAREST)
        return torch.from_numpy(resized).to(device)
    return tensor
...
class sceneObject:
    ...
    def __init__(self, cfg, obj_id, rgb:torch.tensor, depth:torch.tensor, mask:torch.tensor, bbox_2d:torch.tensor, t_wc:torch.tensor, live_frame_id) -> None:
        ...
        # Resize depth and mask to match RGB dimensions
        target_shape = rgb.shape[:2]
        depth = resize_tensor_if_needed(depth, target_shape, rgb.device)
        mask = resize_tensor_if_needed(mask, target_shape, rgb.device)
        
        if depth.shape != target_shape:
            print(f"Resized depth to: {depth.shape}")
