...
class NICE_SLAM():
    ...
    def __init__(self, cfg, args):
        ...
        # CUDA 텐서를 CPU에서 생성하고 공유
        for key, val in self.shared_c.items():
            val = val.cpu()  # CPU에 유지
            val.share_memory_()
            self.shared_c[key] = val
        
        self.shared_decoders = self.shared_decoders.cpu()  # CPU에 유지
    ...
    def tracking(self, rank):
        ...
        # shared_decoders를 GPU로 이동
        self.shared_decoders = self.shared_decoders.to(device)
        
        # shared_c를 GPU로 이동
        for key, val in self.shared_c.items():
            self.shared_c[key] = val.to(device)
